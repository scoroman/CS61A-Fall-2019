...
def taxicab(a, b):
    ...
    streetDistance = abs(street(a) - street(b))
    avenueDistance = abs(avenue(a) - avenue(b)) 

    finalDistance = streetDistance + avenueDistance
...
def balanced(m):
    ...
    x = length(a) * total_weight(end(a))
    y = length(b) * total_weight(end(b))
     
    if(x == y): 
        return is_mobile(mobile(a, b))
    else: 
        return False  """
...
def make_withdraw(balance, password):
    ...
    count = 0
    passwords_list = []
    def withdraw(amount, checkPassword): 
        nonlocal balance, password, passwords_list, count

        if(count >= 3):
            return f"Your account is locked. Attempts: {passwords_list}"

        if(checkPassword != password):
            passwords_list.append(checkPassword)
            count += 1 
