...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if(self.go_backward):
            destination = self.place.entrance # change destination
            if(isinstance(destination, Hive)): # Checking if the bee is in Hive or not
...
def apply_effect(effect, bee, duration):
    ...
    def curr_action(colony):
        ...
        if counter < duration:
            counter += 1
            return effect(last_action, bee) (colony)
        else: 
            return last_action(colony)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if not target.have_been_scared:
            target.have_been_scared = True
