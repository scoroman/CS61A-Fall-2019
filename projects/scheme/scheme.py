...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(eval_first)
        if isinstance (eval_first, MacroProcedure):
            return scheme_eval(eval_first.apply_macro(rest, env), env)
        operators = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(eval_first, operators, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len (vals) > len(formals):
            raise SchemeError("The number of arguments do not match the number of formal parameters")
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        "*** YOUR CODE HERE ***"
        if self.use_env: 
            # adding the current envirFonment to the last argument of Python list
            python_args.append(env)
        try: 
            return self.fn(*python_args)
        except TypeError: 
            raise SchemeError ("Wrong # of arguments were passed")
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, lambda_procedure) # putting our expression in the define pattern
        return target.first # Basically just returning the name
        # END PROBLEM 9
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    content_Body = expressions.rest
    return LambdaProcedure(formals, content_Body, env)
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            reValue = scheme_eval(expressions.first, env, True)
        else:
            reValue = scheme_eval(expressions.first, env)
        if scheme_falsep (reValue): 
            return False
        expressions = expressions.rest
    return reValue
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    formals, vals, bind = nil,nil, bindings
